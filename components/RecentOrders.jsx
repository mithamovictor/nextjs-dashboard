...
const RecentOrders = () => {
    return (
        <div className="w-full col-span-1 relative lg:h-[70vh] h-[50vh] m-auto p-4 border rounded-lg bg-white overflow-y-scroll">
          ...
          <ul>
            {data.map((order, idx)=>(
              <li key={idx} className="bg-gray-50 hover:bg-gray-100 rounded-lg my-3 p-2 flex items-center cursor-pointer">
                ...
                <p className="lg:flex md:hidden absolute right-6 text-sm text-gray-500">{order.date}</p>
              </li>
            ))}
          </ul>
        </div>
